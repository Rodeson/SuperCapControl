...
//升降压判断        
    ...
    if(buckboost_control.BUCKBOOST_CAP_V > CAP_V_MAX && buckboost_control.power_dir == 0)
    {
        ...
        buckboost_control.BuckBoostLoopVcompare_PID.out = 0;
    }
...
void BuckBoost_OUT(void)
{
    ...
    BuckBoost_OUTControl(buckboost_control.ratio_pass);
        ...
        __HAL_TIM_SetCompare(&Q3_PWMControl_TIM,Q3_PWMControl_TIM_CH,1000);
        __HAL_TIM_SetCompare(&Q4_PWMControl_TIM,Q4_PWMControl_TIM_CH,0);  
//        __HAL_TIM_SetCompare(&Q1_PWMControl_TIM,Q1_PWMControl_TIM_CH,200);
//        __HAL_TIM_SetCompare(&Q2_PWMControl_TIM,Q2_PWMControl_TIM_CH,400);
//        __HAL_TIM_SetCompare(&Q3_PWMControl_TIM,Q3_PWMControl_TIM_CH,600);
//        __HAL_TIM_SetCompare(&Q4_PWMControl_TIM,Q4_PWMControl_TIM_CH,800); 
#endif    
...
void BuckBoost_Set_Control(void)
{
    ...
    if(buckboost_control.powerbuffer == 0 && buckboost_control.max_power_lim == 0)
    {
        buckboost_control.powerbuffer = 60;
    }
    
    if(buckboost_control.max_power_lim > 100 || buckboost_control.max_power_lim <= 0)
    {
        buckboost_control.max_power_lim = 30;//没收到裁判系统数据时操作
    }
    
    buckboost_control.max_power_lim = buckboost_control.max_power_lim - 2;
    

    
    //缓冲能量低于20J时降低设定功率上限
    if(buckboost_control.powerbuffer <= 20)
    {
        buckboost_control.max_power_lim = buckboost_control.max_power_lim - 5;
    }
    
    
    //计算缓冲能量环
    PID_Calculate(&buckboost_control.Robot_Buffer_PID,buckboost_control.powerbuffer,30.0f);//预留缓冲能量
    if(buckboost_control.Robot_Buffer_PID.out<-3)
    {
        buckboost_control.max_power_lim = buckboost_control.max_power_lim + 3;
    }
    else
    {
        buckboost_control.max_power_lim = buckboost_control.max_power_lim - buckboost_control.Robot_Buffer_PID.out;
    }
    ...
    //buckboost_control.max_power_lim = buckboost_control.max_power_lim -15;
    ...
    //等效计算出 充放电的功率
    //设置充放电限幅
    if(buckboost_control.charge_power > 80)
    {
        buckboost_control.charge_power = 80;
    }
    else if(buckboost_control.charge_power <-200)
    {
        buckboost_control.charge_power = -200;
    }
...
//    else if(buckboost_control.power_dir == 1)
//    {
//        buckboost_control.charge_current = buckboost_control.charge_power / buckboost_control.BUCKBOOST_CAP_V;
//    }
//    else
//    {
//        buckboost_control.charge_current = buckboost_control.charge_power / buckboost_control.BUCKBOOST_IN_V;
...
//主要计算超级电容 电压环 or 电流环 功率控制最后一环
void BuckBoost_PID_Calculate(void)
{
    if(buckboost_control.power_dir == 0)
    {
        //设置最大充电电流为4.0A
        if(buckboost_control.charge_current > 4.0f)
        {
            buckboost_control.charge_current = 4.0f;
        }
    ...
    //放电模式只计算电流环 不计算电压环 需要多少电流给多少电流 上限15A
    else if(buckboost_control.power_dir == 1)
    {
        if(buckboost_control.charge_current < -15.0f)
        {
            buckboost_control.charge_current = -15.0f;
        }
        buckboost_control.cloop = PID_Calculate(&buckboost_control.BuckBoostLoopCcompare_PID,dcdc_in.I_now,buckboost_control.charge_current);
    ...
    if(buckboost_control.ratio_pass > buckboost_control.BUCKBOOST_CAP_V / buckboost_control.BUCKBOOST_IN_V + 0.3f)
    {
        buckboost_control.ratio_pass = buckboost_control.BUCKBOOST_CAP_V / buckboost_control.BUCKBOOST_IN_V + 0.3f;
    }
}
